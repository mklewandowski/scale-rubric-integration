...
As mentioned in "What is Needed to Display Individual and Group Performance Pages", the minimum information required to display Individual and Group Performance Pages is classroom information (class name, student roster), the current rubric, and per-student scoring information for all questions of the current rubric. The rubric itself (or some representation of required rubric fields) is necessary as it contains content like question prompts and scoring prompts that are displayed in the exported reports. The SCALE Rubric site contains a representation (JSON) of ALL current rubrics. We can either map data from the C2C API to the SCALE Rubric site's internally stored rubrics ***OR*** we can have the API return all rubric data (e.g., question prompts, scoring prompts, etc.) required by the Individual and Group Performance Pages.
...
Here is an example of the data passed from a C2C API endpoint. Ids passed from the C2C API match ids the SCALE Rubric site's internally stored rubrics.
```
{
  "rubricId": "id1",
  "class": {
